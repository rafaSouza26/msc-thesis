...
# Version: 2025-05-16 (Single Detailed CSV output, AIC as standard IC, Syntax Fix)
...
# Removed: SUMMARY_RESULTS_CSV_FILE definition
...
for (i in DISTRICT_CODES_TO_PROCESS) {
  ...
  if (iter_result$status == "Not Run") {
    ...
    if (iter_result$status == "Success" && !is.null(fit_model)) {
      ...
      if(is.na(iter_result$n_models_tested) && !is.null(fit_model$results_summary) && is.matrix(fit_model$results_summary)) {
        iter_result$n_models_tested <- nrow(fit_model$results_summary)
      } else if (is.na(iter_result$n_models_tested) && STEPWISE) { 
        iter_result$n_models_tested <- 1 
      } else if (is.na(iter_result$n_models_tested) && !STEPWISE){ 
        iter_result$n_models_tested <- (MAX_P+1)*(MAX_Q+1)
      }
...
if(nrow(results_df) > 0) {
  ...
  # Removed the block for creating and saving SUMMARY_RESULTS_CSV_FILE
}
...
if(nrow(results_df) > 0) {
  ...
  for(dyn_col in existing_xreg_cols_in_df){
    if(any(!is.na(results_df[[dyn_col]]))) { # Check if column has any non-NA data
      cols_to_print <- c(cols_to_print, dyn_col)
    }
  }
  # Ensure columns to print actually exist in results_df before trying to select them

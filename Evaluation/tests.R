...
source("./ACTS/ingarch.sim.R") # Assumes this is the NEWLY MODIFIED ingarch.sim
...
run_simulation_study_sequential_1_3 <- function() {
  ...
  true_p_dgp <- 1
  true_q_dgp <- 3
  dgp_intercept <- 0.18724697053        # Example intercept
  dgp_betas <- c(0.638013741718721)         # Example beta_1 for p=1
  dgp_alphas <- c(-0.583628692844615, 0.194434539942902, -0.307793633813002) # Example alpha_1, alpha_2, alpha_3 for q=3
  dgp_sigmasq <- 0.0728158879070905      # Example sigmasq (for NB dispersion: distrcoefs (size) = 1/sigmasq) # MODIFIED comment
  
  cat("\nStep 1: Using fixed Data Generating Process (DGP) parameters.\n")
  cat("  True p_order=", true_p_dgp, ", True q_order=", true_q_dgp, ", True sigmasq (alpha for NB variance form mu+alpha*mu^2)=", dgp_sigmasq, "\n") # MODIFIED comment for sigmasq meaning
  ...
  ingarch_params_dgp <- list(
    ...
    past_obs = dgp_betas,    # Corresponds to p_order
    past_mean = dgp_alphas   # Corresponds to q_order
  )
  
  ingarch_model_dgp <- list(
    past_obs = if(true_p_dgp > 0) 1:true_p_dgp else NULL,
    past_mean = if(true_q_dgp > 0) 1:true_q_dgp else NULL,
    external = FALSE # Correctly set for no external regressors
  )
  # MODIFIED: variable name and comments for clarity with 'distrcoefs'
  distrcoefs_param_dgp <- if(!is.na(dgp_sigmasq) && dgp_sigmasq > 0) 1/dgp_sigmasq else {
    warning("Defined dgp_sigmasq is NA, zero, or non-positive; cannot calculate 'distrcoefs' for nbinom.")
    NA
  }
  if(is.na(distrcoefs_param_dgp)) stop("distrcoefs_param_dgp for nbinom is NA. Halting simulation.")
  cat("  Calculated 'distrcoefs' parameter for nbinom simulation (traditional 'size' = 1/sigmasq):", distrcoefs_param_dgp, "\n") # MODIFIED message
  ...
  for(i in 1:num_simulations) {
    ...
    sim_result <- ingarch.sim(n = sim_length,
                              ...
                              distrcoefs = distrcoefs_param_dgp, # MODIFIED: 'size' changed to 'distrcoefs'
                              n_start = 100)
  ...
  populate_results_from_fit <- function(fit_object, template_data,
    ...
    if (!is.null(fit_object) && is.null(fit_object$error) && inherits(fit_object, "tsglm")) {
      ...
      if (!is.null(fit_object$distr) && fit_object$distr == "nbinom") {
        ...
        } else if (!is.null(fit_object$distrcoefs) && !is.null(fit_object$distrcoefs) && is.numeric(fit_object$distrcoefs) && fit_object$distrcoefs > 0) { # Assuming distrcoefs is the size directly for now
          populated_data$sigmasq <- 1 / as.numeric(fit_object$distrcoefs)[1]
        } else if (!is.null(fit_object$distrcoefs) && is.list(fit_object$distrcoefs) && "size" %in% names(fit_object$distrcoefs) && is.numeric(fit_object$distrcoefs$size) && fit_object$distrcoefs$size > 0) { # If distrcoefs is a list with 'size'
  ...
  convert_to_df_with_bind_rows <- function(list_of_result_lists_input, method_name_str, template_for_cols, target_cols_config) {
    list_of_1row_dfs <- lapply(1:length(list_of_result_lists_input), function(j) {
      ...
      for(col_name_template in names(template_for_cols)){
        if(col_name_template %in% names(current_row_data_list) && !is.null(current_row_data_list[[col_name_template]])){
          ...
          if(length(val_to_assign) > 1 && !is.list(val_to_assign)) val_to_assign <- val_to_assign[1] 
    ...
    if (length(list_of_1row_dfs_filtered) > 0) {
      ...
      return(suppressWarnings(dplyr::bind_rows(list_of_1row_dfs_aligned))) 
    } else { 
  ...
  output_dir_name <- "ingarch_dgp13_sequential_results" 
  ...
  if (nrow(results_df) > 0) {
    ...
    if(nrow(order_freq) > 0) {
      ...
      print(as.data.frame(top_orders)) 
